...
#===-- binary_analysis.oy - Binary static analysis
#--------------------------------------==//
#
...
#===----------------------------------------------------------------------===//
#
# This implements the protobuf structure to hold the analysis data
#
#===----------------------------------------------------------------------===//
...
import sys
sys.path.insert(0, '/home/sdasgup3/Github/remill/tools/mcsema/tools/mcsema_disass/ida')
...
def parse_os_defs_file(df):
  ...
  for l in df.readlines():
    ...
    if l.startswith('DATA:'):
      ...
      if conv == "C":
        realconv = binary_analysis_data_pb2.ExternalFunction.CallerCleanup
      elif conv == "E":
        realconv = binary_analysis_data_pb2.ExternalFunction.CalleeCleanup
      elif conv == "F":
        realconv = binary_analysis_data_pb2.ExternalFunction.FastCall
      else:
...
_REFERENCE_OPERAND_TYPE = {
  Reference.IMMEDIATE: binary_analysis_data_pb2.CodeReference.ImmediateOperand,
  Reference.DISPLACEMENT: binary_analysis_data_pb2.CodeReference.MemoryDisplacementOperand,
  Reference.MEMORY: binary_analysis_data_pb2.CodeReference.MemoryOperand,
  Reference.CODE: binary_analysis_data_pb2.CodeReference.ControlFlowOperand,
}
...
def reference_target_type(ref):
  ...
  if ref.ea in EXTERNAL_VARS_TO_RECOVER:
    return binary_analysis_data_pb2.CodeReference.DataTarget

  # TODO(pag): 
  #elif ref.ea in EXTERNAL_FUNCS_TO_RECOVER:
  #  return binary_analysis_data_pb2.CodeReference.CodeTarget
...
def reference_location(ref):
  if ref.ea in EXTERNAL_VARS_TO_RECOVER:
    return binary_analysis_data_pb2.CodeReference.External
  elif ref.ea in EXTERNAL_FUNCS_TO_RECOVER:
    return binary_analysis_data_pb2.CodeReference.External
    ...
    return binary_analysis_data_pb2.CodeReference.Internal
...
_TARGET_NAME = {
  binary_analysis_data_pb2.CodeReference.CodeTarget: "code",
  binary_analysis_data_pb2.CodeReference.DataTarget: "data",
}

_OPERAND_NAME = {
  binary_analysis_data_pb2.CodeReference.ImmediateOperand: "imm",
  binary_analysis_data_pb2.CodeReference.MemoryDisplacementOperand: "disp",
  binary_analysis_data_pb2.CodeReference.MemoryOperand: "mem",
  binary_analysis_data_pb2.CodeReference.ControlFlowOperand: "flow",
}

_LOCATION_NAME = {
  binary_analysis_data_pb2.CodeReference.External: "external",
  binary_analysis_data_pb2.CodeReference.Internal: "internal",
}
...
def recover_instruction_offset_table(I, table):
  ...
  R = I.xrefs.add()
  R.ea = table.offset
  R.operand_type = binary_analysis_data_pb2.CodeReference.OffsetTable
  R.target_type = binary_analysis_data_pb2.CodeReference.DataTarget
  R.location = binary_analysis_data_pb2.CodeReference.Internal
...
def try_recovery_external_flow(I, inst, refs):
  ...
  R.operand_type = binary_analysis_data_pb2.CodeReference.ControlFlowOperand
  R.target_type = binary_analysis_data_pb2.CodeReference.CodeTarget
  R.location = binary_analysis_data_pb2.CodeReference.External
...
def recover_region_cross_references(M, S, seg_ea, seg_end_ea):
  ...
  while next_ea < seg_end_ea:
    ...
    # Probably some really small number.
      ...
      if is_tls(target_ea):
        X.target_fixup_kind = binary_analysis_data_pb2.DataReference.OffsetFromThreadBase
      ...
      # will be an absolute address to the targeted variable/function.
        ...
        X.target_fixup_kind = binary_analysis_data_pb2.DataReference.Absolute
        ...
        if target_name not in EMAP_DATA and target_name not in EMAP:
          ...
          if idc.isCode(target_flags):
            DEBUG("WARNING: Adding external {} at {:x} as an external code reference".format(
                target_name, ea))
            EMAP[target_name] = (16, binary_analysis_data_pb2.ExternalFunction.CallerCleanup, "N", None)
...
def recover_module(entrypoint, gvar_infile = None):
  ...
  if gvar_infile is not None:
    GM = binary_analysis_data_pb2.Module()
