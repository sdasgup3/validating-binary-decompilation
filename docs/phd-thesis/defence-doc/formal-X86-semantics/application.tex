...
Moreover, the reported performance of the applications is not 
optimized\footnote{All these applications are realized based on the  generic 
tools offered by the employed \K semantic framework and  we do not work for 
improving their runtime, which is not impressive at time of writing this 
paper.}, and there is room for improvement, e.g., by providing custom 
abstractions and lemmas specific to x86-64, similarly to~\cite{Park:2018}.}
...
\input{formal-X86-semantics/figures/sum2n.tex}
...
Specifically, it specifies that \reg{rdi} holds the value $N$ without being 
updated during the program execution, and 
%\reg{rax} will have the expected return value.
\reg{rax}, which is pre-conditioned on \emph{any} symbolic value (denoted by 
\m{mi(64, \_:Int):MInt}), is ensured to have the expected return value.

The \s{memstate}~ cell specifies the relevant part of the memory omitting 
others (denoted by ``...'').
It specifies the stack memory addresses \mem{-8}{rbp} and \mem{-4}{rbp} 
corresponding to \m{n} and \m{s}, respectively. Specifically, the stack memory 
addresses \s{-4(\%rbp)} and \s{-8(\%rbp)} start with \emph{any} state  and 
ensures to have respectively the correct sum and $0$ when the program 
terminates. 
The \m{requires}~ clause specifies the condition of $N$ that prevents the 
arithmetic overflow.
...
\input{formal-X86-semantics/figures/histarbug.tex}
...
%<regstate>...
%    "RIP" |-> ( ptr (symloc(-1, 0), mi(64, 0))  => mi(64, 4294967295)) //@ensure
%    "RSP" |-> ( ptr (symloc(1, 128), mi(64, 60))  => ptr(symloc(1, 128), mi(64, 
%    64))) //@ensure
%...</regstate>
%<memstate>... 
% // b will be stored
...
% 
% // Value of of
% 84 |-> byte ( 0 , mi(64, 0)) //@ensure
% 85 |-> byte ( 1 , mi(64, 0)) //@ensure
% 86 |-> byte ( 2 , mi(64, 0)) //@ensure
% 87 |-> byte ( 3 , mi(64, 0)) //@ensure
%...</memstate>
...
%endmodule
%\end{lstlisting}
%\caption{\K specification to drive symbolic execution on code snippet at Figure 
%\ref{fig:histar}.}
%\label{lst:CSHISTAR}
%\end{figure}
