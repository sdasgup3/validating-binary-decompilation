...
\todo[inline]{We are interested only in the validating the translation of instruction semantics and not in validating other decompilation efforts like variable/type recovery}
...
\todo[inline]{Most of the existing approaches are ether validating single instructions or validating program using a heavy weight equivalence checker. Program-level validating has its own important (give the motivating example)}
