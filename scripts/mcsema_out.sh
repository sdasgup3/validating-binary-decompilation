...
#mcsema-lift-4.0 --os linux --arch amd64 --cfg $bin.cfg --output $bin.bc -disable_register_forwarding -disable_dead_store_elimination # now mcsema have -enable_register_forwarding
mcsema-lift-4.0 --os linux --arch amd64 --cfg $bin.cfg --output $bin.bc -disable_dead_store_elimination -disable_optimizer_O3
llvm-dis $bin.bc -o $bin.unoptimized.ll
mcsema-lift-4.0 --os linux --arch amd64 --cfg $bin.cfg --output $bin.bc -disable_optimizer_O3
llvm-dis $bin.bc -o $bin.dead_stores_eliminated.ll
mcsema-lift-4.0 --os linux --arch amd64 --cfg $bin.cfg --output $bin.bc
llvm-dis $bin.bc -o $bin.optimized.ll
