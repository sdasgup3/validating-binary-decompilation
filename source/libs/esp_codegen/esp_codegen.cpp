...
void execute_shell_cmd(const string &cmd, bool printCmd = false) {
  if (printCmd)
    llvm::errs() << cmd << "\n";
  if (system(cmd.c_str())) {
    errs() << "Failed!!";
...
static int bmatch(const string &qOptFile, const string &qFunc,
                  const string &tOptFile, const string &tFunc) {
   ...
   */
  std::unique_ptr<Module> TMod(parseIRFile(tOptFile, Err, Context));
  if (!TMod) {
    Err.print("imatch", errs(), /*showColors=*/true);
    return 2;
  }

  std::unique_ptr<Module> SMod(parseIRFile(qOptFile, Err, Context));
  if (!SMod) {
    Err.print("imatch", errs(), /*showColors=*/true);
    return 2;
  }
...
static bool baselineMatcherDriver(Module &qModule, const string &qFunc,
                                  const string &tgtFile, const string &tFunc) {
  ...
  // string qOutFile = Out + "/test" + ".baseline" + ".query.ll";
  string qOptFile = Out + "/test" + ".baseline" + ".query.opt.ll";
  string qExtractFile = Out + "/test" + ".baseline" + ".query.extract.ll";
  // string tOutFile = Out + "/test" + ".baseline" + ".target.ll";
  string tOptFile = Out + "/test" + ".baseline" + ".target.opt.ll";
  string tExtractFile = Out + "/test" + ".baseline" + ".target.extract.ll";

  processInput(qFunc, qFile, qOptFile, qExtractFile, tFunc, tFile, tOptFile,
               tExtractFile);

  auto res = bmatch(qOptFile, qFunc, tOptFile, tFunc);
...
static void codeGen(const map<Function *, vector<CallInst *>> &workList,
  ...
  for (auto p : workList) {
    ...
    FunctionCallee hookFunc =
        M.getOrInsertFunction(TargetFunc + "_hook", hookFT);
    /*
...
bool codeGenUsingMatcher(
    Module &M, const string &TargetFile, const string &TargetFunc,
    std::map<Function *, std::vector<CallInst *>> &workList) {
  /*
...
//         << "] from target file: " << TargetFile << "\n";
  ...
  std::map<Function *, std::vector<CallInst *>> workList;
  if (codeGenUsingMatcher(M, TargetFile, TargetFunc, workList))
    codeGen(workList, M, TargetFunc);

  return true;
}

// bool codeGenUsingBaselineMatcher(
//     Module &M, const string &TargetFile, const string &TargetFunc,
//     std::map<Function *, std::vector<CallInst *>> &workList) {
//   /*
//    ** Iterate over the functions in query/application module M to be matched
...
// }
//
// bool codeGenUsingIterativeMatcher(
//     Module &M, const string &TargetFile, const string &TargetFunc,
//     const string &Out,
//     std::map<Function *, std::vector<CallInst *>> &workList) {
//   /*
...
//     FQ = &Func;
//
//     errs() << "\n\nMatching (using iterative matcher): " <<
//     FQ->getName().st// r()
//            << " with Target " << TargetFunc << "\n";
//     if (iterativePruningMatcherDriver(M, FQ->getName(), TargetFile,
//     TargetFunc,
//                                       Out)) {
//       errs() << "Match Found\n";
//       for (User *U : FQ->users()) {
