...
Value *max_stack_height::get_init_xsp_or_rsp(Function *F, bool option) {
  ...
  for (auto &I : *EB) {
    if (NULL != (gep_inst = dyn_cast<GetElementPtrInst>(&I))) {
      if(true == gep_inst->hasAllConstantIndices() && 2 ==
  gep_inst->getNumIndices()) {
        ...
        if(idx_1->equalsInt(expected_value_1) &&
  idx_2->equalsInt(expected_value_2)) {
          return gep_inst;
        }
      }
    }
...
bool max_stack_height::initialize_framework() {
  ...
  assert((llvm_alloca_inst_rsp && llvm_alloca_inst_rbp) &&
         "Max Stack height: One of llvm_alloca_inst_rsp or "
         "llvm_alloca_inst_rbp is null\n");
...
// InstMap:  < Value* , <height, atrrib>  > where attrib: <bool isRSP, bool
// isValid>
...
OUT[pred_bb_y]::max_disp_rsp)
        //min is used as values are negative and we need the max negative
displacement.
