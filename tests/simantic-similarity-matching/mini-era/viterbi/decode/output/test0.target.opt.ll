...
entry:
  ...
  tail call void @viterbi_chunks_init_generic() #2
...
while.body:                                       ; preds = %reset.exit, %if.end19
  %n_decoded.045 = phi i32 [ %n_decoded.3, %if.end19 ], [ 0, %reset.exit ]
  %out_count.044 = phi i32 [ %out_count.1, %if.end19 ], [ 0, %reset.exit ]
  %in_count.043 = phi i32 [ %inc20, %if.end19 ], [ 0, %reset.exit ]
...
if.then5:                                         ; preds = %if.then
  ...
  %call6 = call zeroext i8 @viterbi_get_output_generic(i8* getelementptr inbounds ([64 x i8], [64 x i8]* @d_metric0_generic, i64 0, i64 0), i8* getelementptr inbounds ([64 x i8], [64 x i8]* @d_path0_generic, i64 0, i64 0), i32 %5, i8* nonnull %c) #2
...
while.end:                                        ; preds = %if.end19, %reset.exit
  %n_decoded.0.lcssa = phi i32 [ 0, %reset.exit ], [ %n_decoded.3, %if.end19 ]
